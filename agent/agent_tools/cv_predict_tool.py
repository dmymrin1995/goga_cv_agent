...
class CVPredictInput(BaseModel):
    ...
    found_clss: str = Field(
        description="""
            ...
            Если пользователь не указывает конкретные классы используй значение по умолчанию.
        """
    )
    files: str = Field(
        description="""
...
class CVPredictTool(BaseTool):
    ...
    description ="""
        ...
        В качестве аргументов принимает два значения:
            found_clss: str - строка с пречеслением всех классов, необходимых пользователю через запятую или None
